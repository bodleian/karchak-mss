...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0"
    ...
    xmlns:local="/" exclude-result-prefixes="xs local tei" version="3.0">
    ...
    <!-- Script to generate a TEI file for each row in a spreadsheet of Tibetan manuscript descriptions
         ...
         java -Xmx1G -cp ../saxon/saxon9he.jar net.sf.saxon.Transform -it:Main -xsl:tei-from-spreadsheet.xsl infile=driver_collection_batch_2.tsv nextmsid=635
    -->
    ...
    <xsl:template name="CreateTEI">
        ...
        <xsl:if test="string-length($shelfmarknum) gt 0">
            <!-- <xsl:variable name="shelfmark" as="xs:string" select="concat('MS. Driver. ', $shelfmarknum)"/> -->
            <xsl:variable name="shelfmark" as="xs:string" select="$shelfmarknum"/>
            <!-- <xsl:variable name="tempbox" as="xs:string" select="$fields[2]"/> -->
            ...
            <xsl:variable name="script" as="xs:string" select="$fields[6]"/>
            <xsl:variable name="langscript" as="xs:string" select="$fields[7]"/>
            ...
            <xsl:variable name="dimensions" as="xs:string" select="$fields[8]"/>
            ...
            <xsl:variable name="subject1" as="xs:string" select="replace($fields[9], '\.\s*$', '')"/>
            <xsl:variable name="link1" as="xs:string" select="substring-after($fields[10], 'lccn.loc.gov/')"/>
            <xsl:variable name="subjectlink1" as="xs:string" select="if ($subject1[not(. eq '')]) then concat($subject1, '%', $link1) else ''"/>
            <xsl:variable name="subject2" as="xs:string" select="replace($fields[11], '\.\s*$', '')"/>
            <xsl:variable name="link2" as="xs:string" select="substring-after($fields[12], 'lccn.loc.gov/')"/>
            ...
            <xsl:result-document href="../../driver/{$filename}.xml" method="xml" encoding="UTF-8" indent="yes">
                ...
                <xsl:value-of select="$newline"/>
                <!-- <xsl:comment>
                    <xsl:text> Temporary box: </xsl:text>
                    <xsl:value-of select="$tempbox"/>
                    <xsl:text></xsl:text>
                </xsl:comment>
                <xsl:value-of select="$newline"/>
                <xsl:value-of select="$newline"/> -->
                <TEI xmlns="http://www.tei-c.org/ns/1.0"
                    xmlns:tei="http://www.tei-c.org/ns/1.0" xml:id="manuscript_{ $msid }">
                    <teiHeader>
                        <fileDesc>
                            <titleStmt>
                                <title>
                            ...
                            <sourceDesc>
                                <msDesc xml:lang="en" xml:id="{ $filename }">
                                    ...
                                    <history>
                                        ...
                                        <acquisition when="2016">
                                            <persName role="fmo" key="person_286478621">John E. Stapleton Driver</persName> collection, donated by <persName role="dnr" key="person_n2017054713">Prof Felix Driver</persName> to the Bodleian Libraries in 2016</acquisition>
                                    </history>
                                    <additional>
                                        <adminInfo>
                                            <availability status="restricted">
                                    ...
                                    <keywords scheme="#LCSH">
                                        <list>
                                            <xsl:for-each select="distinct-values(($subject1, $subject2)[not(. eq '')])">
                                                ...
                                                <xsl:variable name="matchedauthority" as="element(tei:item)*" select="$authoritysubjects[tei:term/normalize-space(lower-case(translate(string(.), '.-', ''))) = normalize-space(lower-case(translate($subjectterm, '.-', '')))]"/>

                                                <xsl:choose>
                                                    <xsl:when test="$matchedauthority">
                                                        <!-- Get the id of the first matched item - in case there are duplicates -->
                                                        <xsl:variable name="matchedid" select="$matchedauthority[1]/@xml:id"/>
                                                        <item>
                                                            <term key="{ $matchedid }">
