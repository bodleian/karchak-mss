...
<xsl:stylesheet 
    ...
    <xsl:template name="CreateTEI">
        ...
        <xsl:if test="string-length($shelfmarknum) gt 0">
            <xsl:variable name="shelfmark" as="xs:string" select="concat('MS. Driver. a. ', $shelfmarknum)"/>
            <xsl:variable name="tempbox" as="xs:string" select="$fields[2]"/>
            <xsl:variable name="notitle" as="xs:boolean" select="starts-with($fields[3], 'nt')"/>
            <xsl:variable name="romanizedtitle" as="xs:string" select="if (not($notitle)) then $fields[3] else ''"/>
            <xsl:variable name="romanizedincipit" as="xs:string" select="if ($notitle) then substring-after($fields[3], ' ') else ''"/>
            <xsl:variable name="tibetantitle" as="xs:string" select="if (not($notitle)) then $fields[4] else ''"/>
            <xsl:variable name="tibetanincipit" as="xs:string" select="if ($notitle) then $fields[4] else ''"/>
            <xsl:variable name="extent" as="xs:string" select="$fields[5]"/>
            <xsl:variable name="types" as="xs:string*" select="tokenize($fields[6], '[^A-Za-z]+')"/>
            <xsl:variable name="script" as="xs:string" select="$fields[7]"/>
            <xsl:variable name="langscript" as="xs:string" select="$fields[8]"/>
            ...
            <xsl:variable name="dimensions" as="xs:string" select="$fields[9]"/>
            ...
            <xsl:variable name="subject1" as="xs:string" select="replace($fields[10], '\.\s*$', '')"/>
                                    ...
                                    <keywords scheme="#LCSH">
                                        <list>
                                            <xsl:for-each select="distinct-values(($subject1, $subject2)[not(. eq '')])">
                                                ...
                                                <xsl:variable name="matchedauthority" as="element(tei:item)*" select="$authoritysubjects[tei:term/normalize-space(lower-case(translate(string(.), '.-', ''))) = lower-case(translate($subjectterm, '.-', ''))]"/>
                                                <xsl:choose>
