...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:template name="CreateTEI">
        ...
        <xsl:variable name="leafsize" as="xs:string" select="$fields[4]"/>
    ...
    <xsl:function name="local:lookupAuthority" as="xs:string*">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:message>Cannot find entry in <xsl:value-of select="$authorityname"/>
 for: <xsl:value-of select="string-join($values, ' | ')"/>
            </xsl:message>
        </xsl:otherwise>
    </xsl:choose>
</xsl:function>
