...
if [ ! "$5" == "noindex" ]; then
    ...
    if [ ! $1 == "organizations.xquery" ]; then
        ...
        curl -X POST -fsS "http://${4}:8983/solr/karchak-mss/update?commit=true" --data-binary "<delete><query>type:${3}</query></delete>" -H "Content-Type: text/xml" 1>> $LOGFILE 2>> $LOGFILE
    fi
    ...
    echo "Processing $3 records finished. Skipped sending to Solr in $5 mode."
    exit 0;
fi
