...
java -Xmx1G -cp "saxon/saxon9he.jar" net.sf.saxon.Query -q:update-subjects-authority-file.xquery -o:"$TEMPFILE2" 2>> $LOGFILE
if [ $? -gt 0 ]; then
    ...
    cp "$TEMPFILE2" "$AUTHORITYDIR/subjects_additions.xml" 2>> $LOGFILE
    if [ $? -gt 0 ]; then
        echo "Cannot write to authority folder. Check permissions on $AUTHORITYDIR and re-run."
        exit 1;
    else
