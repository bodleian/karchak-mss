...
declare variable $base := doc("../authority/persons.xml")/tei:TEI/tei:text/tei:body/tei:listPerson/tei:person[@xml:id];
...
declare variable $highestcurrentkey := max(for $k in $currentkeys return if (starts-with($k, 'person_')) then xs:integer(replace($k, '\D', '')) else () );
            ...
            <listPerson>
{
    ...
    let $localpeoplefrompreviousrun := (
        for $p in $additions[starts-with(@xml:id, 'person_')]
            return
            <person>
                { $p/@* }
                { $p/* }
    ...
    return (
        ...
        $linebreak,
        $linebreak,
        comment{' TODO: Review the following entries, update their key attributes in the TEI files, then cut and paste them into persons.xml '},
        $linebreak,
